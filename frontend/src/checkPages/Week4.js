...
export default function Week4({ user }) {
  ...
  const fetchWeek = async () => {
    const response = await fetch('http://localhost:4000/api/progress/week4')
  ...
  const fetchRecord = async () => {
    const response = await fetch(
      'http://localhost:4000/api/progress/week4/' + user.user.phone_number
    )
    const json = await response.json()
    ...
    <div>
            <div className='mt-20'>
                ...
                <div>
                    {record && (
                        <div>
                            {record.name}---{record.task1 ? 'done' : 'notdone'}---{record.task2 ? 'done' : 'notdone'}---{record.link1}---{record.link2}
            ...
            <div>
                {week && week.map((x) => (
                    <p key={x._id}>
                        {x._id}---{x.name}---{x.task1 ? 'done' : 'notdone'}---{x.task2 ? 'done' : 'notdone'}---{x.link1}---{x.link2}
