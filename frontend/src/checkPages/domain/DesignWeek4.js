...
export default function DesignWeek4({ user }) {
  ...
  const fetchWeek = async () => {
    const response = await fetch(BaseUrl + '/api/progress/designweek4')
  ...
  const fetchRecord = async () => {
    ...
    const response = await fetch(
      BaseUrl + '/api/progress/designweek4/' + user.phone_number
    )
    const json = await response.json()
      ...
      <div className="lg:col-span-9 ">
        {false ? (<>
          <div className="lg:mt-20 w-full">
            <h1 className=" text-center text-[#00CDB7] lg:text-6xl text-2xl relative top-14 py-10 font-bold">DESIGN WEEK 4</h1>
