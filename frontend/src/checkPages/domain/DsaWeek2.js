...
export default function DsaWeek2({ user }) {
  ...
  const fetchWeek = async () => {
    const response = await fetch('http://localhost:4000/api/progress/dsaweek2')
  ...
  const fetchRecord = async () => {
    ...
    const response = await fetch(
      'http://localhost:4000/api/progress/dsaweek2/' + user.phone_number
    )
    const json = await response.json()
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex-1 ml-64 p-4 mt-20">
        <h1 className="text-xl font-bold">DSA WEEK 2</h1>
          ...
          {week &&
            week.map((x) => (
              <p key={x._id}>
                ...
                {x.task2 ? 'done' : 'notdone'}---{x.task3 ? 'done' : 'notdone'}
                ---
                {x.task4 ? 'done' : 'notdone'}---{x.task5 ? 'done' : 'notdone'}
                ---
                {x.task6 ? 'done' : 'notdone'}---{x.link1}---{x.link2}
              </p>
            ))}
        </div>
