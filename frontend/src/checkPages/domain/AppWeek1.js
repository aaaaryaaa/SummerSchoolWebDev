...
export default function AppWeek1({ user }) {
  ...
  const fetchWeek = async () => {
    const response = await fetch('http://localhost:4000/api/progress/appweek1')
  ...
  const fetchRecord = async () => {
    ...
    const response = await fetch(
      'http://localhost:4000/api/progress/appweek1/' + user.phone_number
    )
    const json = await response.json()
    ...
    <div className="grid grid-cols-10 w-full">
      ...
      <div className="col-span-9">
        <div className="mt-24 w-full">
          <h1 className=" text-center text-6xl py-10">APP WEEK 1</h1>
