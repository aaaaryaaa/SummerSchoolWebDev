...
export default function DsaWeek3({ user }) {
  ...
  const fetchWeek = async () => {
    const response = await fetch('http://localhost:4000/api/progress/dsaweek3')
  ...
  const fetchRecord = async () => {
    ...
    const response = await fetch(
      'http://localhost:4000/api/progress/dsaweek3/' + user.phone_number
    )
    const json = await response.json()
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex-1 ml-64 p-4 mt-20">
        <h1 className="text-xl font-bold">DSA WEEK 3</h1>
